...
module.exports.run = async (bot, msg: Message, args) => {
	if(!(msg.channel as TextChannel).nsfw) return require('../../util/errMsg.js').run(bot, msg, true, `You can\'t use this in a SFW channel. Use this in a NSFW channel like: ${msg.guild.channels.cache.array().filter(c => c.type == "text").filter((c: TextChannel) => c.nsfw).map(c => c.name).join(" ")}`);
