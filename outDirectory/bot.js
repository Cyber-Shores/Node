...
const Bot = new machina_ts_1.Machina(process.env.TOKEN, "&");
...
const qM = require("./util/queueMessage");
    ...
    folders.forEach(item => {
        fs.readdir(`./cmds/${item}`, (err, files) => {
            ...
            tsfiles.forEach((f, i) => {
                ...
                console.log(`${i + 1} ${f}!`);
                Bot.loadCommands(...Object.values(props));
            });
        });
    });
});
...
Bot.client.once('ready', async () => {
    ...
    Bot.client.guilds.cache.get("708879213951844393").channels.cache.get("735331472679895170").send(new Discord.MessageEmbed({ title: "cheeze its" }));
});
// #endregion
...
Bot.client.on('message', async (msg) => {
    ...
    if (msg.content === '<prefix>') {
        const req = await GuildModel.findOne({ id: msg.guild.id });
        if (!req)
            return require('./util/errMsg').run(Bot, msg, true, 'Something went wrong while loading your servers prefix/suffix\nPlease report this to our support server: https://discord.gg/GUvk7Qu');
        const prefixembed = new machina_ts_1.MachinaMessage({
            title: 'Server Prefix & Suffiix:',
            description: `Prefix:  ${req.prefix}\nSuffix:  ${req.suffix}\n`,
            color: msg.member.displayHexColor,
            footer: {
        ...
        const setprefixembed = await new machina_ts_1.MachinaMessage({
            title: 'Prefix Reset!',
            description: 'Prefix: <\nSuffix: >\n',
            color: msg.member.displayHexColor,
            footer: {
    ...
    if (!!process.env.PREFIX) {
        req.prefix = process.env.PREFIX;
    }
    if (!!process.env.SUFFIX) {
        req.suffix = process.env.SUFFIX;
    }
    try {
    ...
    if (command.reason == "no commands available")
        return machina_ts_1.Machina.noCommands(msg, command.extra);
    else if (command.reason == "permission check passed multiple")
        return machina_ts_1.Machina.multipleCommands(msg, machina_ts_1.arrify(command.value));
    if (command.value)
        machina_ts_1.arrify(command.value).forEach(f => f(Bot, msg));
});
// #endregion
// #region  Node Network
Bot.client.on('message', async (msg) => {
    ...
    if (msg.channel.type === 'dm')
        return;
    if (!msg.channel.name.startsWith("node-"))
        return;
    let msgChannel = msg.channel;
    // creates new node network message
    const embed = new machina_ts_1.MachinaMessage({
        author: {
    ...
    if (attachment)
        embed.msg.embeds[0].setImage(attachment.url);
    if (msg.channel.name == "node-network") {
        Bot.client.guilds.cache.array().forEach(g => g.channels.cache.filter(c => c.name == 'node-network' && c != msg.channel).array().forEach((c) => c.send(embed)));
    }
    else {
        Bot.client.channels.cache.filter((c) => c.name == msgChannel.name && c.topic == msgChannel.topic && c != msgChannel).array().forEach((c) => c.send(embed));
    }
});
Bot.client.on('channelCreate', async (channel) => {
    if (channel.name != "node-network")
        return;
    if (channel.guild.channels.cache.filter(chn => chn.name == "node-network").size > 1)
        return;
    function getDate() {
    ...
    Bot.client.guilds.cache.array().forEach(async (g) => {
        try {
            ...
            if (message.author.id == Bot.client.user.id) {
                ...
                if (g.channels.cache.find(c => c.name == 'node-network'))
                    g.channels.cache.find(c => c.name == 'node-network').topic = "Welcome to the Node Network v1.1! Say Hi, and be friendly.";
            }
        }
        catch (e) {
            console.log(e.stack);
...
Bot.client.on('channelDelete', async (channel) => {
    ...
    if (channel.name != "node-network")
        return;
    if (channel.guild.channels.cache.filter(chn => chn.name == "node-network").size > 0)
        return;
    function getDate() {
        const today = new Date();
        const dd = String(today.getDate()).padStart(2, '0');
