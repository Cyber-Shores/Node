...
module.exports.run = async (bot, msg, args) => {
    if (!args[0])
        return require('../../util/errMsg.js').run(bot, msg, true, 'Please provide a search term.');
    const m = await msg.channel.send('```Generating image...```');
    const options = {
        // eslint-disable-next-line no-inline-comments
        url: encodeURI('http://results.dogpile.com/serp?qc=images&q=' + args.join(' ')),
        method: 'GET',
        headers: {
            'Accept': 'text/html',
            'User-Agent': 'Chrome',
    ...
    request(options, function (error, response, responseBody) {
        if (error)
            return require('../../util/errMsg.js').run(bot, msg, false, 'Something went wrong, please report this in our support server:\nhttps://discord.gg/GUvk7Qu');
