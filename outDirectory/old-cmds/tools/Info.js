...
module.exports.run = async (bot, msg, args) => {
    ...
    if (args[0] == 'bio') {
        ...
        const bioarray = args.filter((_, i) => i > 0).join(" "); // (strings.map(function(f) { return f.substring(f.indexOf(' ') + 1);}).join(' '));
        if (bioarray.length > 150)
            return require('../../util/errMsg').run(bot, msg, true, 'Server bio can not be longer than 150 characters');
