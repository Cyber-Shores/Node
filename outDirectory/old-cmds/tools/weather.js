...
module.exports.run = async (bot, msg, args) => {
    if (!args[0])
        return require('../../util/errMsg.js').run(bot, msg, true, 'Please provide a location to search.');
    const m = await msg.channel.send('```Generating weather data...```');
    let str = '';
    let woeid;
    ...
    await axios.get(str).then(weather => {
        function genIcon() {
            const state = weather.data.consolidated_weather[0].weather_state_abbr;
            switch (state) {
                case 'sn':
        ...
        function calcTemp() {
            const locale = msg.guild.region.toLowerCase();
        ...
        const embed = new MessageEmbed()
            ...
            .addFields({ name: 'Temperature', value: calcTemp(), inline: true }, { name: 'Weather', value: weather.data.consolidated_weather[0].weather_state_name, inline: true }, { name: 'Humidity', value: `${weather.data.consolidated_weather[0].humidity}%`, inline: true }, { name: 'Wind Speed', value: calcWind(), inline: true }, { name: 'Wind Direction', value: weather.data.consolidated_weather[0].wind_direction_compass, inline: true }, { name: 'Visibility', value: calcVis(), inline: true })
...
module.exports.help = {
    ...
    usage: `${config.pref}weather [city]${config.suff}`,
