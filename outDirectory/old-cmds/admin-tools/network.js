...
module.exports.run = async (bot, msg, args) => {
    ...
    if (!args[0]) {
        ...
        if (!args[1])
            return require('../../util/errMsg.js').run(bot, msg, true, `Must initialize the custom network with a name and 5 digit numeric password\ne.g. ${config.pref}network Test 00000${config.suff}`);
        if (args[0].length > 95)
            return require('../../util/errMsg.js').run(bot, msg, true, "Name must be 95 characters or less");
        if (args[1].length != 5)
            return require('../../util/errMsg.js').run(bot, msg, true, "Password must contain exactly 5 digits");
        for (let i = 0; i < args[1].length; i++) {
            if (isNaN(parseInt(args[1].substring(i, i + 1))))
                return require('../../util/errMsg.js').run(bot, msg, true, "Password may only contain numbers");
        }
        let channel;
        try {
