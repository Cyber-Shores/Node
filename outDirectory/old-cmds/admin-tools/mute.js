...
module.exports.run = async (bot, msg, args) => {
    ...
    if (USER.id == msg.author.id)
        return require('../../util/errMsg.js').run(bot, msg, false, 'You cannot mute youself!');
    if (USER.roles.highest.position >= msg.member.roles.highest.position)
        return require('../../util/errMsg.js').run(bot, msg, false, 'Target user is of higher ranking then you!');
    ...
    if (USER.roles.cache.has(ROLE.id))
        return require('../../util/errMsg.js').run(bot, msg, false, `${USER} is already muted!`);
